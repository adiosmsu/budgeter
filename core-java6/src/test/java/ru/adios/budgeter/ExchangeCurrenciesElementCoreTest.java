...
public class ExchangeCurrenciesElementCoreTest {
    ...
    public void testSubmit() throws Exception {
        ...
        final TestCheckedRunnable runnable = new TestCheckedRunnable() {
            @Override
    ...
    private void testSubmitWith(Bundle bundle, String caseName) throws Exception {
        ...
        ratesRepo.addRate(TestUtils.JULY_3RD_2015, Units.RUB,
                CurrencyUnit.USD, CurrencyRatesProvider.Static.reverseRate(BigDecimal.valueOf(55.5))); // to know exact value we set "natural" rate ourselves
        ...
        assertEquals(
                ...
                CurrencyExchangeEvent.builder()
                        ...
                        .setRate(CurrencyRatesProvider.Static.reverseRate(BigDecimal.valueOf(56.)).round(mc))
        ...
        ratesRepo.addRate(TestUtils.DAY_BF_YESTER, CurrencyUnit.USD,
                Units.BTC, CurrencyRatesProvider.Static.reverseRate(BigDecimal.valueOf(250.))); // to know exact value we set "natural" rate ourselves
        core.setPersonalMoneyExchange(true);
        ...
        final Optional<CurrencyExchangeEvent> btcExc = bundle.currencyExchangeEvents().streamForDay(TestUtils.DAY_BF_YESTER)
                .reduce(new BinaryOperator<CurrencyExchangeEvent>() {
                    @Override
                    public CurrencyExchangeEvent apply(CurrencyExchangeEvent event, CurrencyExchangeEvent event2) {
                        return event.bought.getCurrencyUnit().equals(Units.BTC) && event.sold.getCurrencyUnit().equals(CurrencyUnit.USD) ? event : event2;
                    }
                });
        ...
        assertEquals(
                ...
                CurrencyExchangeEvent.builder()
                        ...
                        .setRate(CurrencyRatesProvider.Static.reverseRate(BigDecimal.valueOf(260.)).round(mc))
                        .setTimestamp(DateTimeUtils.convertToCurrentZone(TestUtils.DAY_BF_YESTER.inner))
        ...
        final Optional<FundsMutationEvent> btcExcMutation = bundle.fundsMutationEvents().streamForDay(TestUtils.DAY_BF_YESTER)
                .reduce(new BinaryOperator<FundsMutationEvent>() {
                    @Override
                    public FundsMutationEvent apply(FundsMutationEvent event, FundsMutationEvent event2) {
                        return event.amount.getCurrencyUnit().equals(Units.BTC) ? event : event2;
                    }
                });
