...
public class FundsMutationElementCoreTest extends AbstractFundsTester {
    ...
    protected void actualTest(Bundle bundle,
        ...
        final Optional<FundsMutationEvent> savedWorkHour = bundle.fundsMutationEvents().streamForDay(TestUtils.JULY_3RD_2015).filter(new Predicate<FundsMutationEvent>() {
            @Override
            public boolean test(FundsMutationEvent event) {
                return event.subject.equals(workSubj);
            }
        }).findFirst();
        ...
        final Optional<FundsMutationEvent> workConversionDelta = bundle.fundsMutationEvents().streamForDay(TestUtils.JULY_3RD_2015)
                .filter(new Predicate<FundsMutationEvent>() {
                    @Override
                    public boolean test(FundsMutationEvent event) {
                        return event.subject.equals(FundsMutationSubject.getCurrencyConversionDifferenceSubject(accounter.fundsMutationSubjectRepo()));
                    }
                })
                .findFirst();
        ...
        rubAccount = treasury.getAccountForName(rubAccount.name).get();
        final Optional<FundsMutationEvent> savedCard = bundle.fundsMutationEvents().streamForDay(TestUtils.DAY_BF_YESTER).filter(new Predicate<FundsMutationEvent>() {
            @Override
            public boolean test(FundsMutationEvent event) {
                return event.subject.equals(cardSubj);
            }
        }).findFirst();
        ...
        final Optional<FundsMutationEvent> cardDelta = bundle.fundsMutationEvents().streamForDay(TestUtils.DAY_BF_YESTER)
                .filter(new Predicate<FundsMutationEvent>() {
                    @Override
                    public boolean test(FundsMutationEvent event) {
                        return event.subject.equals(fundsMutSubj);
                    }
                })
                .findFirst();
        ...
        rubAccount = treasury.getAccountForName(rubAccount.name).get();
        final Optional<FundsMutationEvent> savedGuitar = bundle.fundsMutationEvents().streamForDay(TestUtils.YESTERDAY).filter(new Predicate<FundsMutationEvent>() {
            @Override
            public boolean test(FundsMutationEvent event) {
                return event.subject.equals(guitarSubj);
            }
        }).findFirst();
