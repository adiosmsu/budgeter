...
public final class SourcingBundle implements Bundle {
    ...
    private void executeMultipleSql(final JdbcTemplate jdbcTemplate, String[] createTableSql, @Nullable Logger logger) {
        for (final String sql : createTableSql) {
            if (sql != null) {
                if (logger != null) {
                    logger.info(sql);
                }
                if (jdbcConnector.transactionalSupport != null) {
                    jdbcConnector.transactionalSupport.runWithTransaction(new Runnable() {
                        @Override
                        public void run() {
    ...
    public void tryExecuteInTransaction(Runnable code) {
        def.tryExecuteInTransaction(code);
    }
